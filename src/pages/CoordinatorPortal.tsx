...
function AgencyInvoices() {
  ...
  return <>
    ...
    <section className="panel billing-explainer">
      ...
      <div><ShieldCheck size={20} /><span><strong>Payout protection</strong><small>Unit price and TaskBridge margin are shown before VAT. VAT is added only where applicable on the invoice.</small></span></div>
    </section>
    <section className="panel table-panel">
      ...
      <div className="responsive-table"><table><thead><tr><th>Task</th><th>Payment route</th><th>Unit price</th><th>Settlement</th><th>Handyman</th></tr></thead><tbody>{filteredCharges.map((charge) => <tr key={charge.id}><td><strong>{charge.taskId}</strong><small>{charge.category} / {humanize(charge.taskStatus)} / {formatDate(charge.createdAt, true)}</small></td><td><strong>{paymentRouteLabel(charge.paymentRoute)}</strong><small>{humanize(charge.paymentStatus)}</small></td><td><strong>{money(charge.totalAmount, charge.currency)}</strong><small>Handyman payout {money(charge.handymanAmount, charge.currency)} / TaskBridge margin {money(chargeMarginTotal(charge), charge.currency)}</small><small>VAT excluded; added on invoice where applicable.</small></td><td><StatusBadge status={charge.settlementStatus}>{humanize(charge.settlementStatus)}</StatusBadge>{charge.settlementReference && <small>{charge.settlementReference}</small>}{charge.settlementDueAt && <small>Due {formatDate(charge.settlementDueAt)}</small>}</td><td><strong>{charge.handymanName || "Not recorded"}</strong>{charge.settlementNotes && <small>{charge.settlementNotes}</small>}</td></tr>)}</tbody></table></div>
